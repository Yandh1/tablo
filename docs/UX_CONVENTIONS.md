...
### Default and edited layouts

Every persisted project has two independent diagram position maps:

- `Default layout` is generated by the frontend layout algorithm when table
  topology changes. It is deterministic and not directly draggable.
- `Edited layout` begins as a copy of the current default positions and is then
  changed only by deliberate user layout actions.

Expose a visible, keyboard-operable two-option control near the diagram controls.
The labels are `Default layout` and `Edited layout`; do not hide the distinction
behind icons. The selected layout is conveyed with text and accessible state,
not color alone.

- Switching layouts changes positions only. It does not change source, parse
  state, canonical identities, selection, or editor focus.
- Entering Edited layout for the first time copies the current default positions.
- Node dragging is enabled only in Edited layout. Persist the result on drag end,
  never on every pointer movement.
- Adding a table recomputes default positions. Existing edited positions remain
  unchanged, and a new table receives its default position in the edited map.
- Deleting a table removes its position from both maps.
- Name and column edits that do not change topology do not re-layout either map.
- `Reset edited layout` copies the current default map only after explicit
  confirmation because it overwrites user positioning.
- A save failure or revision conflict keeps the local edited map visible and
  exposes Retry or conflict resolution. It never silently falls back to default.
- React Flow owns the applied position transforms. Layout switching and autosave
  must not add a second DOM transform owner.
...
- Default and Edited layout maps survive reload and remain independent.
- Default layout cannot be changed by dragging.
- A new table receives a generated default position without resetting existing edited positions.
- Edited positions are persisted after drag end, not during continuous pointer movement.
